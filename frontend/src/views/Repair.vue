...
const form = reactive(emptyForm()); const filters = reactive({ sn: '', keyword: '', technician: '', date_from: '', date_to: '' }); const records = ref(JSON.parse(localStorage.getItem('gpu-repair-query-history') || '[]')); const groupedRecords = computed(() => { const groups = new Map(); records.value.forEach(record => { const key = (record.sn || '').trim().toUpperCase(); if (!groups.has(key)) groups.set(key, { ...record, history: [] }); else groups.get(key).history.push(record) }); return Array.from(groups.values()) }); const fileList = ref([]); const loading = ref(false); const saving = ref(false); const detail = ref(null); const detailVisible = ref(false); const currentErrorLoading = ref(false); const searchedCurrentError = ref(null); let errorRequestSerial = 0

...
-- 娉ㄩ噴
COMMENT ON TABLE gpu_test_records IS 'GPU娴嬭瘯璁板綍琛?;
COMMENT ON COLUMN gpu_test_records.id IS '璁板綍ID';
COMMENT ON COLUMN gpu_test_records.qc_id IS '鍗曟QC浠诲姟ID';
COMMENT ON COLUMN gpu_test_records.sn IS 'GPU搴忓垪鍙?;
COMMENT ON COLUMN gpu_test_records.status IS '娴嬭瘯鐘舵€?PASSED/FAILED)';
COMMENT ON COLUMN gpu_test_records.error_code IS '閿欒浠ｇ爜';
COMMENT ON COLUMN gpu_test_records.error_message IS '閿欒淇℃伅';
COMMENT ON COLUMN gpu_test_records.slot_info IS '鎻掓Ы淇℃伅';
COMMENT ON COLUMN gpu_test_records.server_ip IS '娴嬭瘯鏈嶅姟鍣↖P';
COMMENT ON COLUMN gpu_test_records.qc_timestamp IS 'QC娴嬭瘯鏃堕棿';
COMMENT ON COLUMN gpu_test_records.test_log IS '鍘熷娴嬭瘯鏃ュ織鍐呭';
COMMENT ON COLUMN gpu_test_records.created_at IS '鍒涘缓鏃堕棿';
COMMENT ON COLUMN gpu_test_records.updated_at IS '鏇存柊鏃堕棿';
...
CREATE TABLE IF NOT EXISTS gpu_repair_records (
    ...
    status VARCHAR(40) NOT NULL DEFAULT '寰呮祴璇?,
...
ALTER TABLE gpu_repair_records DROP COLUMN IF EXISTS brand;
ALTER TABLE gpu_repair_records DROP COLUMN IF EXISTS core;
ALTER TABLE gpu_repair_records DROP COLUMN IF EXISTS memory;
